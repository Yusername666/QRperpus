    var metode
     ...
     function scanSiswa() {
        Swal.fire({
            title: 'Metode Scan QRcode Diubah!',
            text: 'Silahkan Scan Kartu QR-Siswa !',
    ...
    function scanBuku() {
        Swal.fire({
            title: 'Metode Scan QRcode Diubah!',
            text: 'Silahkan Scan Label QR-Book !',
            allowOutsideClick: false,
            icon: 'info'
        })
        metode = 'buku'
    ...
    (function(undefined) {
        ...
        var args = {
            ...
            //ajax ambil data controller
            var url

            if (metode == "siswa") {
                url = "Peminjaman/scanNis"
            } else if (metode == "buku") {
                url = "Peminjaman/scanBuku"
            } else {
                url = "null"
            }
            $.ajax({
                ...
                success: function (a) {
                    ...
                    if (send == null) {
                        ...
                        alert('Scanning QRCODE Berhasil!')
                        if (metode == "siswa") {
                            $("#nis").val(send.nis)
                            $("#nama_lengkap").val(send.nama_lengkap)
                        } else if (metode == "buku") {
                            $("#kd_inventaris").val(send.kd_inventaris)
                            $("#judul_buku").val(send.judul_buku)
                            $("#isbn").val(send.isbn)
                        }
                    }
                },
                error: function (a, t, e) {
                    ...
                    Swal.fire({
                        title: 'Metode Scan QRcode INVALID!',
                        allowOutsideClick: false,
                        icon: 'error'
                    })
                  //   $('#pesan-hasil').html('<div class="alert alert-danger" role="alert">SCAN GAGAL, METODE INVALID</div>');
                  //   setTimeout(function () {
                  //     $("#pesan-hasil").html('');
                  // }, 3000);
              },
